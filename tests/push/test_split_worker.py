...
class SplitWorkerAsyncTests(object):
    ...
    async def test_on_error(self, mocker):
        ...
        split_worker = SplitWorkerAsync(handler_sync, mocker.Mock(), q, mocker.Mock(), mocker.Mock(), mocker.Mock())
        split_worker.start()
        assert split_worker.is_running()
...
#        assert change_number_received == 123456789
        ...
        self._feature_flag = None
        async def put(feature_flag):
            self._feature_flag = feature_flag

        self.new_change_number = 0
        async def set_change_number(new_change_number):
            self.new_change_number = new_change_number
